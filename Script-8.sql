...
with c1 as
(select distinct bp.flight_id, count(s.seat_no) as c1
from seats s 
join boarding_passes bp on bp.seat_no = s.seat_no 
group by bp.flight_id),
 c2 as 
(select distinct bp.flight_id, count(bp.seat_no) as c2
from boarding_passes bp
group by bp.flight_id),
c3 as (select c1.flight_id, (c1.c1 - c2.c2) as c3
from c2
join c1 on c2.flight_id = c1.flight_id
)
select c1.flight_id, c3.c3 "свободные", 
round(c3.c3::numeric / c1.c1::numeric, 3) * 100 as "процент",
f.actual_departure::date,
f.departure_airport,
count (bp.ticket_no) over (partition by f.departure_airport order by f.actual_departure::date)
...
join airports_data ad on f.arrivar_airport = ad.airport_code 
